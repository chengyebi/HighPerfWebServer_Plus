...
1. 将单 Reactor 架构升级为主从 Reactor：
   主线程只负责连接接入与分发，多个子 Reactor 线程各自维护独立 epoll 实例和连接集合，减少主线程热点竞争
1. 新增服务配置模块，支持命令行启动参数：
   `--host`、`--port`、`--threads`、`--resources`
   其中 `--threads` 在当前版本中表示 Sub Reactor 数量
...
- 主线程作为 Main Reactor，只负责监听 socket 的 `accept`
- 新连接按 round-robin 分发给多个 Sub Reactor
- 每个 Sub Reactor 独立维护 `epoll + connection map + idle timer heap`
- 连接的读写、协议解析、超时回收都在所属 Sub Reactor 线程内完成
- 避免单 Reactor 下主线程既做接入又做海量连接管理造成瓶颈
...
- 接入路径使用 `accept4` 直接获取 non-blocking fd，减少额外系统调用
...
3. 为什么主从 Reactor 比单 Reactor 更适合在连接数上去以后继续扩展
4. 为什么静态文件发送选择 `sendfile`
5. 如何避免 fd 复用引起的 ABA 清理问题
6. 为什么一个高并发服务器还需要 `/metrics` 和 `/healthz`
7. 空闲连接为什么要做超时回收，它对 fd 资源和高并发稳定性有什么意义
8. 为什么空闲连接回收更适合用最小堆定时器，而不是每轮遍历所有连接
9. 这个项目是如何从“高性能网络 Demo”演进成“完整服务架构 Plus 版”的
